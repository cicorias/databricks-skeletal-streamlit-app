...
Conventions for Python 3.12+ scripts used in automation, tooling, and CLI applications.
...
Use Python 3.12+ syntax with built-in generics.
...
2. PEP 723 inline script metadata (if applicable)
3. Future imports: `from __future__ import annotations`
4. Imports: standard library, third-party, local (separated by blank lines)
5. Constants and exit codes
6. Module-level logger
7. Helper functions
8. Parser creation function
9. Logging configuration function
10. Run logic function
11. Main entry point
12. Module guard: `if __name__ == "__main__": sys.exit(main())`
...
Place before module docstring. Run with `uv run script.py`.

## Linting and Formatting

Use **ruff** for both linting and formatting. Run from the `databricks/` directory:
...
# Lint and auto-fix
cd databricks && uv run ruff check --fix src notebooks

# Format (check only)
cd databricks && uv run ruff format --check src notebooks

# Format (apply)
cd databricks && uv run ruff format src notebooks
```
...
- **F821** — suppressed in notebooks via `--per-file-ignores="notebooks/**:F821"` (Databricks globals like `spark`, `dbutils`, `display` are injected at runtime)
- **Line length** — 120 characters (configured in `databricks/src/pyproject.toml`)
